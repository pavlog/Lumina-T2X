...
class DiT_LlamaP2P(nn.Module):
    ...
    def forward(self, x, t, y, input_latents):
        ...
        drop_out  = self.training
        t = self.t_embedder(t)  # (N, D)
        y = self.y_embedder(y,drop_out)  # (N, D)
        adaln_input = t + y
...
class DiT_LlamaP2P_2(nn.Module):
    """
    Diffusion model with a Transformer backbone.
    """
    ...
    def __init__(
        ...
        self.t_embedder = ParallelTimestepEmbedder(min(dim, 1024))
        self.t_embedder2 = ParallelTimestepEmbedder(min(dimStage2, 1024))
        self.y_embedder = ParallelLabelEmbedder(num_classes, min(dim, 1024), class_dropout_prob)
        self.y_embedder2 = ParallelLabelEmbedder(num_classes, min(dimStage2, 1024), class_dropout_prob)
        ...
        self.layersPre = nn.ModuleList(
            [
                ...
                for layer_id in range(n_layersNoise)
            ]
        )
        ...
        self.layers2 = nn.ModuleList(
            [
                ...
                for layer_id in range(n_layersInputs)
            ]
        )
        ...
        self.layers = nn.ModuleList(
            [
                TransformerBlockSandwichNorm2(
                    layer_id, dimStage2, n_heads, n_kv_heads, multiple_of, ffn_dim_multiplier, norm_eps, qk_norm
                )
                for layer_id in range(n_layers)
            ]
        )
        
        self.final_layer = ParallelFinalLayer(dimStage2, patch_size, self.out_channels)
        ...
        self.freqs_cis_stage2 = DiT_Llama.precompute_freqs_cis(
            dimStage2 // n_heads,
            384,
            # theta=100.
        )
    ...
    def getfreqs_cis_stage2(self, x: torch.Tensor) -> torch.Tensor:
        B, C, H, W = x.size()
        # assert (H, W) == (self.input_size, self.input_size)
        pH = pW = self.patch_size
        return self.freqs_cis_stage2[: H // pH, : W // pW].flatten(0, 1).unsqueeze(0)
    ...
    def forward(self, x, t, y, input_latents):
        ...
        y: (N,) tensor of class labels
        """
        freqs_cis2_st2 = self.getfreqs_cis_stage2(x) # stage 2 can have different freqs_cis_stage2 
        freqs_cis2_st2 = freqs_cis2_st2.to(x.device)
        ...
        y1 = self.y_embedder(y,drop_out)  # (N, D)
        t2 = self.t_embedder2(t)  # (N, D)
        y2 = self.y_embedder2(y,drop_out)  # (N, D)
        adaln_input = t1 + y1
        adaln_input2 = t2 + y2
        ...
        for layer in self.layers2:
            x2 = layer(x2, freqs_cis2, adaln_input=y1)

        x = torch.cat([x,x2],dim=2)

        for layer in self.layers:
            x = layer(x, freqs_cis2_st2, adaln_input=adaln_input2)

        x = self.final_layer(x, adaln_input2)
        x = self.unpatchify(x, H, W)  # (N, out_channels, H, W)
        if self.learn_sigma:
            x, _ = x.chunk(2, dim=1)
        return x
...
def DiT_Llama_2B_patch2(**kwargs):
    return DiT_Llama(patch_size=2, dim=2304, n_layers=24, n_heads=32, **kwargs)
...
def DiT_Llama_600_patch2_Actions3(**kwargs):
    return DiT_LlamaP2P_2(in_channels=4,patch_size=2, dim=512,dimStage2=1024,n_layersNoise=4,n_layersInputs=4,n_layers=12, n_heads=32,out_channels=4,**kwargs)
...
if __name__ == "__main__":
    ...
    model = DiT_Llama_600_patch2_Actions2(num_classes=100,qk_norm=True)
    #model = DiT_Llama_600_patch2_Actions3(num_classes=100,qk_norm=True)
    model.to("cuda")
